...
public class RxPermissions {
    ...
    private Observable<Permission> request_(final String... permissions) {
        ...
        for (String permission : permissions) {
            if (isGranted(permission)) {
                // Already granted, or not Android M. Return a granted Permission object.
                list.add(Observable.just(new Permission(permission, Permission.STATUS_GRANTED)));
                continue;
            }

            if (isRevoked(permission)) {
                // Revoked by a policy, return a denied Permission object.
                list.add(Observable.just(new Permission(permission, Permission.STATUS_REVOKED)));
                continue;
            }
        ...
        if (!unrequestedPermissions.isEmpty()) {
            startShadowActivity(unrequestedPermissions.toArray(new String[unrequestedPermissions.size()]));
        }
    ...
    void startShadowActivity(String[] permissions) {
        Intent intent = new Intent(mCtx, ActivityForPermissions.class);
    ...
    void onRequestPermissionsResult(int requestCode, String permissions[], int[] grantResults) {
        for (int i = 0, size = permissions.length; i < size; i++) {
            ...
            if (subject == null) {
                // No subject found
                throw new IllegalStateException("RxPermissions.onRequestPermissionsResult invoked but didn't " +
                        "find the corresponding permission request.");
            }
            mSubjects.remove(permissions[i]);
            boolean granted = grantResults[i] == PackageManager.PERMISSION_GRANTED;
            subject.onNext(new Permission(permissions[i], granted ? Permission.STATUS_GRANTED : Permission.STATUS_DENIED));
            subject.onCompleted();
        }
    }
}